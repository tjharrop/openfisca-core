...
def set_input_divide_by_period(holder, period, array):
    ...
    sub_period = period.start.period(cached_period_unit)
    all_sub_periods_count = 0
    filled_sub_periods_count = 0
    while sub_period.start < after_instant:
        existing_array = holder.get_array(sub_period)
        if existing_array is not None:
            remaining_array -= existing_array
            filled_sub_periods_count += 1
        all_sub_periods_count += 1
        sub_period = sub_period.offset(1)

    # Cache the input data
    if all_sub_periods_count != filled_sub_periods_count:
        divided_array = remaining_array / (all_sub_periods_count - filled_sub_periods_count)
        sub_period = period.start.period(cached_period_unit)
        while sub_period.start < after_instant:
        ...
        divided_array = array.copy() / all_sub_periods_count
        sub_period = period.start.period(cached_period_unit)
        while sub_period.start < after_instant:
            if holder.get_array(sub_period) is None:
                holder._set(sub_period, divided_array)
            else:
                holder._set(sub_period, divided_array + holder.get_array(sub_period))
            sub_period = sub_period.offset(1)
